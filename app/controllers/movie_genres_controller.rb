class MovieGenresController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @movie_genre.save
        format.html { redirect_to movie_genres_url, notice: "Movie genre was successfully created." }
        format.json { render :index, status: :created, location: @movie_genre }
      else
  ...
  def update
    respond_to do |format|
      if @movie_genre.update(movie_genre_params)
        ...
        format.json { render :index, status: :ok, location: @movie_genre }
      else
